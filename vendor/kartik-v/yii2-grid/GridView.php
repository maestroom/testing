...
HTML;
    ...
    public function run()
    {
        ...
        $this->containerOptions['tabindex'] = '-1';
    ...
    public function renderExport()
    {
        ...
        $title = ($icon == '') ? $title : "<em class='{$iconPrefix}{$icon}'></em> {$title}";
    ...
    protected function initToggleData()
    {
        ...
        if (!empty($icon)) {
            $label = "<em class='glyphicon glyphicon-{$icon}'></em> " . $label;
        }
    ...
    protected function beginPjax()
    {
        ...
        if (!empty($postPjaxJs)) {
			...
			$postPjaxJs1 = "if($('.table-responsive').length>0 && $('.kv-panel-pager').length>0) { 
		...
		$('.kv-grid-container').height(grid_height);
            ...
            $js .= ".off('{$event}').on('{$event}', function(){{$postPjaxJs} {$postPjaxJs1}})";
        }
    ...
    protected function registerAssets()
    {
        ...
        /* start: custom code for resizing grid height according to screen resolution */
        $customJs = "if($('.table-responsive').length>0 && $('.kv-panel-pager').length>0) { 
		if($('.sub-heading').length > 0) { 
		var grid_height = $('.table-responsive').height()-$('.kv-panel-pager').height()-$('.sub-heading').height()-5; 
		}else{ 
		var grid_height = $('.table-responsive').height()-$('.kv-panel-pager').height()-5; 
        ...
        $view->registerJs($customJs, View::POS_READY);
        /* end: custom code for resizing grid height according to screen resolution */
