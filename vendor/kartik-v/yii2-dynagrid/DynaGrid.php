...
class DynaGrid extends Widget
{
    ...
    public function init()
    {
        ...
        /*dynagird required issue model load*/
        $post_data=Yii::$app->request->post();
        $this->_model->attributes = Yii::$app->request->post();
        if(isset($post_data['DynaGridConfig']['sortId']) && $post_data['DynaGridConfig']['sortId']!=""){
			$this->_model->sortId=$post_data['DynaGridConfig']['sortId'];
		}else{
			$this->_model->sortId=NULL;
		}
        $this->_isSubmit = !empty($_POST[$this->_requestSubmit]) && $this->_model->validate();
        ...
        //echo "<pre>",print_r($this->gridOptions),"</pre>";die;
        $this->initGrid();
    }
    ...
    protected function applySort()
    {
        if (!empty($this->_detailConfig[DynaGridStore::STORE_SORT])) {
            //echo "here",print_r($this->_detailConfig[DynaGridStore::STORE_SORT]);die;
            ...
            if(is_array($sort->defaultOrder) && $sort->enableMultiSort == true){
                $sort->defaultOrder = array_merge($sort->defaultOrder,$this->_detailConfig[
                DynaGridStore::STORE_SORT]);
            }else{
                $sort->defaultOrder = $this->_detailConfig[
                DynaGridStore::STORE_SORT];
            }
            $sort->getOrders(true);
    ...
    protected function setToggleButton($cat)
    {
        ...
        if ($cat == DynaGridStore::STORE_GRID) {
            $this->toggleButtonGrid = ArrayHelper::merge([
                'label' => '<i class="glyphicon glyphicon-wrench"></i><span class="hide">Personalize grid settings</span>',
                'title' => Yii::t('kvdynagrid', 'Personalize grid settings'),
                'data-pjax' => false
            ], $this->toggleButtonGrid);
        } else {
            $this->$setting = ArrayHelper::merge([
                'label' => "<i class='glyphicon glyphicon-{$cat}'></i><span class='hide'>Save / edit grid {$cat}</span>",
