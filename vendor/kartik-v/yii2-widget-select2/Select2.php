...
class Select2 extends InputWidget
{
     ...
     * - `selectLabel`: string, the markup to be shown to select all records. Defaults to `<em class="glyphicon
     *     glyphicon-unchecked"></em> Select all`.
     * - `unselectLabel`: string, the markup to be shown to unselect all records. Defaults to `<em class="glyphicon
     *     glyphicon-checked"></em> Unselect all`.
    ...
    public function renderWidget()
    {
        ...
        if(!isset($this->pluginOptions['allowClear']) || $this->pluginOptions['allowClear'] != ""){
        ...
        if (!isset($this->data)) {
            if (!isset($this->value) && !isset($this->initValueText)) {
                ...
                /* added condition $this->value!="" for fixing error in case of blank value in multiselect filter value in grid */
                $key = isset($this->value) && $this->value!="" ? $this->value : ($multiple ? [] : '');
                $val = isset($this->initValueText) ? $this->initValueText : $key;
                $this->data = $multiple ? (is_array($key)?array_combine($key, $val):array_combine(array($key), $val)) : [$key => $val];
            }
        }
    ...
    protected function renderToggleAll()
    {
        ...
        $settings = array_replace_recursive([
            'selectLabel' => '<em class="glyphicon glyphicon-unchecked"></em>' . Yii::t('kvselect', 'Select all'),
            'unselectLabel' => '<em class="glyphicon glyphicon-check"></em>' . Yii::t('kvselect', 'Unselect all'),
            'selectOptions' => [],
            'unselectOptions' => [],
    ...
    protected function renderInput()
    {
        ...
        if(isset($this->options['nolabel']) && $this->options['nolabel']==true)
			$input =$this->getInput('dropDownList', true);
        else
        {
            if(isset($this->options['prompt']) && $this->options['prompt']!="")
                $lbltext = $this->options['prompt'];
            elseif(isset($this->options['placeholder']) && $this->options['placeholder']!="")
                $lbltext = $this->options['placeholder'];
            elseif(isset($this->options['title']) && $this->options['title']!="")
                $lbltext = $this->options['title'];
            else
                $lbltext = "select filter";
			$input = '<label for="'.$this->options['id'].'" style="display:none"><span>'.$lbltext.'</span></label>'.$this->getInput('dropDownList', true);
        }
