...
class ActiveField extends Component
{
    ...
    public function label($label = null, $options = [])
    {
        ...
        if ($this->model->isAttributeRequired($this->attribute)) {
            $span="<span class='hidden'>Required</span>";
        }
        $options = array_merge($this->labelOptions, $options);
        if ($label !== null) {
            $options['label'] = $label . $span;
        }/*else{
            echo "<pre>",print_r($options),$this->attribute,"</pre>";//die;
        }*/
        
        $this->parts['{label}'] = Html::activeLabel($this->model, $this->attribute, $options);
        if ($label === null) {
            $this->parts['{label}'] = str_replace("</label>", $span."</label>", $this->parts['{label}']);
        }
