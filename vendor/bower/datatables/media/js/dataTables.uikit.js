	...
	var attach = function( container, buttons ) {
		...
		for ( i=0, ien=buttons.length ; i<ien ; i++ ) {
			...
			if ( $.isArray( button ) ) {
				...
				switch ( button ) {
					case 'ellipsis':
						btnDisplay = '<em class="uk-icon-ellipsis-h"></em>';
						btnClass = 'uk-disabled disabled';
						break;

					case 'first':
						btnDisplay = '<em class="uk-icon-angle-double-left"></em> ' + lang.sFirst;
						btnClass = (page > 0 ?
							'' : ' uk-disabled disabled');
						break;

					case 'previous':
						btnDisplay = '<em class="uk-icon-angle-left"></em> ' + lang.sPrevious;
						btnClass = (page > 0 ?
							'' : 'uk-disabled disabled');
						break;

					case 'next':
						btnDisplay = lang.sNext + ' <em class="uk-icon-angle-right"></em>';
						btnClass = (page < pages-1 ?
							'' : 'uk-disabled disabled');
						break;

					case 'last':
						btnDisplay = lang.sLast + ' <em class="uk-icon-angle-double-right"></em>';
						btnClass = (page < pages-1 ?
							'' : ' uk-disabled disabled');
						break;
