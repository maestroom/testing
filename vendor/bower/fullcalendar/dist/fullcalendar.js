...
Grid.mixin({
	...
	bindSegHandlers: function() {
		...
		this.bindSegHandler('focus', this.handleSegEvent);
	},
	...
	handleSegEvent: function(seg, ev) {
		return this.view.trigger('eventFocus', seg.el[0], seg.event, ev); // can return `false` to cancel
	},
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '" tabindex="0"' +
				(event.url ?
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						...
						'>' +
							'<span style="display:inline-block;">' + htmlEscape(startTimeText) + '</span>' + //change due to pdf attr(data-start) by perception team
						'</div>' :
						''
						) +
					(event.title ?
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							var btn_title = '';
							if(buttonName=='prev')
								var btn_title = 'Previous Month';
							if(buttonName=='next')
								var btn_title = 'Next Month';
							if(buttonName=='today')
								var btn_title = 'Highlight Today';
							if(buttonName=='agendaDay')
								var btn_title = 'Day View';
							if(buttonName=='agendaWeek')
								var btn_title = 'Week View';
							if(buttonName=='month')
								var btn_title = 'Month View';	

							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" class="' + classes.join(' ') + '" title="'+btn_title+'">' +
									innerHtml +
								'</button>'
								)
								.click(function(ev) {
