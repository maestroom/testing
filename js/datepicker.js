...
var datePickerController = (function datePickerController() {
    ...
    function datePicker(options) {
        ...
        this.create = function() {
            ...
            if(document.getElementById(this.id).getAttribute("shownodate")==1)
            ...
            /* start: show no date option */
            if(this.shownodate) {
                var lbl = document.createElement('label');
                lbl.setAttribute("for", "chknodate");
                lbl.innerHTML = "No Date";
                lbl.setAttribute("style", "bottom: 6px;left: 125px;position: absolute;z-index: 9999;");
                var chkbx = document.createElement('input');
                chkbx.setAttribute("type","checkbox");
                chkbx.setAttribute("id", "chknodate");
                chkbx.setAttribute("style", "bottom: 12px;left: 175px;position: absolute;z-index: 9999;");
                var inpid = this.id;
                chkbx.onclick = function(e) { 
                    if(this.checked) 
                    { 
                            document.getElementById(inpid).value = "00/00/0000"; 
                            //datePickerController.setEnabledDates("prod_rec_date", '*');
                           //datePickers["prod_rec_date"].setEnabledDates('*');
                    }
                }
                
                this.div.appendChild(lbl);
                this.div.appendChild(chkbx);
            }
            /* end: show no date option */
    ...
    datePicker.prototype.createButton = function() {
        ...
        fontIconHolder.title="Select Date";
    ...
    datePicker.prototype.returnFormattedDate = function(noFocus) {
        ...
        for (elemID in this.formElements) {
            ...
            if (elem.tagName.toLowerCase() == "input") {
                elem.value = fmtDate;
                if(this.shownodate) {
                    document.getElementById('chknodate').checked=false;
                }
            } else {
    ...
    var localeDefaults = {
        ...
        titles: ["Previous month", "Next month", "Previous year", "Next year", "Today", "Show Calendar", "wk", "Week [[%0%]] of [[%1%]]", "Week", "Select a date", "Click \u0026 Drag to move", "Display \u201C[[%0%]]\u201D first", "Go to Today\u2019s date", /*"Disabled date :"*/ "Date Outside of Business Hours", "Show calendar for [[%0%]]"],
        rtl: false,
        firstDayOfWeek: 6,
        imported: false
    };
    ...
    var addDatePicker = function(options) {
        ...
        /*start: added code to clear selected date in input box in custom forms (custodian, instruction, outcome forms) */
        if($('#'+options.id).hasClass('datepickers'))
        {
            if($('#'+options.id).parent().find('#cleardate').length == 0)
            {
                $('#'+options.id).parent().append('<span id="cleardate" class="select2-selection__clear" title="Remove" aria-label="Remove" role="presentation"><span class="sr-only">remove</span><span class="fa fa-times fa-fw fa-1x"></span></span>');
                $('#'+options.id).parent().find('#cleardate').click(function(){
                    $('#'+options.id).val('');
                });
            }
        }
        /*end: added code to clear selected date in input box in custom forms (custodian, instruction, outcome forms) */
