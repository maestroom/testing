...
Admin.formbuilder = {
		...
		properties: function(e)
		{
			$(e).find('a.properties').unbind("click").bind("click",function(){
				...
				var new_label = label.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
															...
															if(!$(this).is(':checked')) {
																$(this).prop("checked",true);	
                                                                    ...
                                                                    if($( "input[name='label']").val() != '') {
                                                                        $lblTxt = $( "input[name='label']").val();
																		$lblTxt = $lblTxt.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
																	}
					...
					$("input[name='label']").on('keyup',function(){
						...
						if($('input[name="properties['+id+'][label]"]').length)
						{
							if($(this).val().replace(/\s/g, '')=="")
								...
								var $on_keypress_lblTxt = $(this).val().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
								$('input[name="properties['+id+'][label]"]').val($on_keypress_lblTxt);
							}
						}
						else
						{
							var $on_keypress_lblTxt = $(this).val().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
							$('.attrs clear '+id).append('<input type="hidden" value="'+$on_keypress_lblTxt+'" class="label" name="properties['+id+'][label]">');
						}
						var $on_keypress_lblTxt = $(this).val().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
					...
					$(".tbodyClass_"+id).sortable({
						...
						stop: function(e,ui) { 
							...
							$(".tbodyClass_"+id+" > tr ").each(function(i){ // new
									...
									if(sorder == "")
										sorder = $(this).find('td:nth-child(1)').html();
									else
										sorder = sorder + ';'  + $(this).find('td:nth-child(1)').html();
						...
						switch (type)
						{
							case 'dropdown':
								...
								for (i in value){
									...
									if(newc != ""){
										if(value[i] != ""){	
											newc += '<option value='+j+' description="'+value[i]+'">'+value[i].substring(0,65);+'</option>';
										}
										else if(newc == ""){
											newc = def;
										}	
										...
										if(value[i] != ""){
											newc = def+'<option value='+j+' description="'+value[i]+'">'+value[i].substring(0,65);+'</option>';
										}else{
											newc = def;
										}
									}
							...
							case 'radio':
								var newc = '';
								for (i in value) newc += '<input type="radio" class="cls_'+id+'" '+required+' aria-label="'+value[i]+'" value="'+value[i]+'" name="temp['+name+'][]"><label for="lbl_chkbx_'+id+'" class="form_label fbg-label"><span class="cls_value_'+id+'"> '+value[i]+' </span></label> <br/>';
								$(rel).html(newc);
								break;
							case 'checkbox':
								var newc = '';
								for (i in value) newc += '<input type="checkbox" class="cls_'+id+'" '+required+' aria-label="'+value[i]+'" value="'+value[i]+'" name="temp['+name+'][]"><label for="lbl_chkbx_'+id+'" class="form_label fbg-label"><span class="cls_value_'+id+'">'+value[i]+'</span></label> <br/>';
								$(rel).html(newc);
								break;
							default: break;
						}
			 ...
			 $('input[name="optionchk_name"]').unbind('change').bind('change',function(){
				    ...
				    if($(this).attr('class') == 'radio_btn' || $(this).attr('class') == 'dropdown_btn'){
				    	...
				    	if($(this).is(':checked')){
				            ...
				            var optTextVal  = $tr.find('input').val();
				            //alert(optTextVal);
...
function addOption(elementId,typeClass,chk2) {
	...
	$('.ui-dialog').each(function(){
    	if($(this).is(":visible")){
	        var optionTxt = $(this).find('#form_builder_properties #optionTextboxAddVal').val();
	        if (optionTxt.replace(/\s/g, '') == '') {
	        	...
	        	if(error == 1){
		        	...
		        	if(allvalues != "") {
		        		$(this).find("#form_builder_properties ."+typeClass).val(allvalues+";"+optionTxt);
		        	} else {
		        		$(this).find("#form_builder_properties ."+typeClass).val(optionTxt);
		        	}
	        ...
	        if($('input[name="optionchk_name"]').attr('class') == 'radio_btn') {
	        	$("#form_builder_panel .cls_"+elementId).removeAttr("checked");
	        	$("#form_builder_panel .cls_"+elementId).each(function(){
	        		if($(this).val() == selvalues){
	        			$(this).attr("checked","checked");
	        		}
	        	});
	        } else if($('input[name="optionchk_name"]').attr('class') == 'dropdown_btn') {
	        	if(selvalues.replace(/\s/g, '') != "") {
	        		$("#form_builder_panel select[name='"+elementId+"'] > option:contains(" + selvalues + ")").attr('selected','selected');
	        	} else {
	        		$("#form_builder_panel select[name='"+elementId+"']").val(0);
	        	}
	        } else {
		...
		$(".tbodyClass_"+elementId).sortable({
			...
			stop: function(e,ui) { 
				...
				$(".tbodyClass_"+elementId+" > tr ").each(function(i){ // new
						...
						if(sorder == "")
							sorder = $(this).find('td:nth-child(2)').html();
						else
							sorder = sorder + ';'  + $(this).find('td:nth-child(2)').html();
				});
