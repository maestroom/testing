    ...
    Dynagrid.prototype = {
        ...
        listen: function () {
            ...
            self.$btnSubmit.off('click').on('click', function () {
                ...
                setTimeout(function () {
                        /* start: ajax code written for ajax grids to hide dynagrid config popup, save changes and reload grid without refreshing page */
                        /*$form.submit();*/
                        $.ajax({
                            type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
                            url         : $form.data('yiiActiveForm').settings.validationUrl, // the url where we want to POST
                            data        : $form.serialize(), // our data object
                            dataType    : 'json', // what type of data do we expect back from the server
                            encode      : true,
                            success     : function(response) {
                                $('#' + self.modalId).hide();
                                $.pjax.defaults.url =$form.data('yiiActiveForm').settings.validationUrl;
                                $.pjax.defaults.push= false;
                                if($('#'+self.dynaGridId+'-pjax'))
                                    $.pjax.reload('#'+self.dynaGridId+'-pjax', $.pjax.defaults);
                                else
                                    window.location.reload();
                            }    
                        });
                    /* end: ajax code written for ajax grids to hide dynagrid config popup, save changes and reload grid without refreshing page */
                }, 1000);
            });
