...
use  app\components\mailer\phpmailer\PHPMailer;
use Yii;
 ...
 */
//require_once(dirname(__FILE__).DIRECTORY_SEPARATOR.'mailer'.DIRECTORY_SEPARATOR.'phpmailer'.DIRECTORY_SEPARATOR.'class.phpmailer.php');
...
class EMailer 
{
   ...
   public function setPathLayouts($value)
   {
      if (!is_string($value) && !preg_match("/[a-z0-9\.]/i"))
         throw new \yii\web\HttpException(404, 'pathLayouts must be a Yii alias path');
   ...
   {
      if (!is_string($value) && !preg_match("/[a-z0-9\.]/i"))
         throw new \yii\web\HttpException(404, 'pathViews must be a Yii alias path');
    ...
    */
	...
	{
		$reflect = new \ReflectionClass($this->_myMailer);
		if (is_object($this->_myMailer) && $reflect->getShortName()==='PHPMailer') return call_user_func_array(array($this->_myMailer, $method), $params);
		else throw new \yii\web\HttpException(404, 'Can not call a method of a non existent object');
	}
    ...
    */
	...
	{
		$reflect = new \ReflectionClass($this->_myMailer);
	   if (is_object($this->_myMailer) && $reflect->getShortName()==='PHPMailer') $this->_myMailer->$name = $value;
	   else throw new \yii\web\HttpException(404,'Can not set a property of a non existent object');
	}
    ...
    */
	...
	{
	   $reflect = new \ReflectionClass($this->_myMailer);
	   if (is_object($this->_myMailer) && $reflect->getShortName()==='PHPMailer') return $this->_myMailer->$name;
	   else throw new \yii\web\HttpException(404, 'Can not access a property of a non existent object');
	}
