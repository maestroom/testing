...
function SkipSearchWord(sw) {
	if (searchWords[sw] != "") {
		if (SkippedWords > 0)
			SkippedOutputStr += ", ";
		SkippedOutputStr += "\"<strong>" + searchWords[sw] + "</strong>\"";
		searchWords[sw] = "";
		SkippedWords++;
	}
}
...
function ZoomShowResults()
{
	...
	{
		if (ZoomInfo == 1)
			document.writeln("<center><p><small>" + STR_POWEREDBY + " <a href=\"http://www.wrensoft.com/zoom/\" zoom_target=\"_blank\"><strong>Zoom Search Engine</strong></a></small></p></center>");
		return;
	}
	...
	while (arrayline < matches && arrayline < result_limit) {
		...
		if (DisplayNumber == 1)
			document.writeln("<strong>" + (arrayline+1) + ".</strong>&nbsp;");
...
function ZoomShowSorting()
{
	...
	if (matches > 1)
	{
		if (UseDateTime == 1)
		{
			...
			if (sort == 1)
				document.writeln("<a href=\"" + SelfURL + "?zoom_query=" + queryForURL + metaParams + "&amp;zoom_page=" + page + "&amp;zoom_per_page=" + per_page + query_zoom_cats + "&amp;zoom_and=" + andq + "&amp;zoom_sort=0\">" + STR_SORTBY_RELEVANCE + "</a> / <strong>" + STR_SORTEDBY_DATE + "</strong>");
			else
				document.writeln("<strong>" + STR_SORTEDBY_RELEVANCE + "</strong> / <a href=\"" + SelfURL + "?zoom_query=" + queryForURL + metaParams + "&amp;zoom_page=" + page + "&amp;zoom_per_page=" + per_page + query_zoom_cats + "&amp;zoom_and=" + andq + "&amp;zoom_sort=1\">" + STR_SORTBY_DATE + "</a>");
			document.writeln("</div>");
		}
	}
}
...
function ZoomShowPageNumbers()
{
	...
	if (ZoomInfo == 1)
		document.writeln("<center><p><small>" + STR_POWEREDBY + " <a href=\"http://www.wrensoft.com/zoom/\" target=\"_blank\"><strong>Zoom Search Engine</strong></a></small></p></center>");
}
