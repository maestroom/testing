...
$('form#{$model->formName()}').on('beforeSubmit', function(e) {
   var form = $(this);
		$.ajax({
            ...
            data   : form.serialize(),
            beforeSend : function()    {
            	$('#submitCustomWordingInstructionFooter').attr('disabled','disabled');
            },
            success: function (response){
				if(response == 'OK')
             		CustomWordingInstructionFooter();
            	else{
                	$('#submitCustomWordingInstructionFooter').removeAttr("disabled");
            	}
            },
            error  : function (){
...
?>
				...
				<div class="x_panel custom-wording-editer">
					<div class="x_content">
						<div id="alerts"></div>
						<?php 
		 							if(isset($instruction_footer_data->fieldtext) && $instruction_footer_data->fieldtext != ''){ 
		 								$model->instruction_footer = $instruction_footer_data->fieldtext;
		 							}
		 							echo $form->field($model, 'instruction_footer',['inputOptions'=>['style'=>"display:block;width:100%;"]])->textArea(['rows'=>5,'maxlength'=>$settings_length['fieldtext']])->label(false);
	 							?>
					</div>
				</div>	
					<!--<div class="x_panel custom-wording-editer">
                            <div class="x_content">
                                ...
                                <div id="editor1" class="editor-container">
                                    <?php echo $model->instruction_footer;?>
                                </div>
                                <br />
                            </div>
                	</div>-->
					<div class="button-set text-right">
						<?= Html::button('Cancel', ['title'=>"Cancel",'class' => 'btn btn-primary','onclick'=>"javascript:CustomWordingInstructionFooterSelect();"]) ?>
             	 		<?= Html::submitButton('Update', ['title'=>"Update",'class' => 'btn btn-primary','id'=>'submitCustomWordingInstructionFooter']) ?>
					</div>
				<?php ActiveForm::end(); ?>
<script>
	/* change evnet */
	...
	$(function () {
		$('#settings-instruction_footer').jqte({source: false});
...
//			});
		...
		$('#editor1').wysiwyg('document').keypress(function(e) {
			...
			var maxlength = parseInt($('#settings-instruction_footer').attr("maxlength"));		 
