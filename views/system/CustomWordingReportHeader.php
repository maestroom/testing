...
$('form#{$model->formName()}').on('beforeSubmit', function(e) {
   var form = $(this);
		$.ajax({
            ...
            data   : form.serialize(),
            beforeSend : function() {
            	$('#submitCustomWordingReportHeader').attr('disabled','disabled');
            },
            success: function (response) {
				if(response == 'OK')
             		CustomWordingReportHeader();
            	else{
                	$('#submitCustomWordingReportHeader').removeAttr("disabled");
            	}
            },
            error  : function () {
...
<div class="x_panel custom-wording-editer">
	<div class="x_content">
		<div id="alerts"></div>
<?php 
				if(isset($report_header_data->fieldtext) && $report_header_data->fieldtext != ''){ 
					$model->report_header = $report_header_data->fieldtext;
				}
				echo $form->field($model, 'report_header',['inputOptions'=>['style'=>"display:block;width:100%;"]])->textArea(['rows'=>5,'maxlength'=>$settings_length['fieldtext']])->label(false);
				?>
	</div>
</div>	
	<!--<div class="x_panel custom-wording-editer">
			<div class="x_content">
				...
				<div id="editor1" class="editor-container">
					<?php echo $model->report_header; ?>
				</div>
				<br />
			</div>
	</div>-->
	<div class="button-set text-right">
		<div class='text-left col-sm-6'>Note: This text will appear right-aligned in the printed version of your Report Exports.</div>
		<?= Html::button('Cancel', ['title'=>"Cancel",'class' => 'btn btn-primary','onclick'=>"javascript:CustomWordingReportHeaderSelect();"]) ?>
		<?= Html::submitButton('Update', ['title'=>"Update",'class' => 'btn btn-primary','id'=>'submitCustomWordingReportHeader']) ?>
	</div>
<?php ActiveForm::end(); ?>
<script>
	/* change evnet */
	...
	$(function () {
		$('#settings-report_header').jqte({source: false});
...
//			});
		...
		$('#editor').unbind('DOMSubtreeModified').bind('DOMSubtreeModified', function(event) {
			$('#settings-report_header').html($(this).html());
		});
		...
		$('#editor1').wysiwyg('document').keypress(function(e) {			
			...
			var maxlength = parseInt($('#settings-report_header').attr("maxlength"));		 
