...
$('form#{$model->formName()}').on('beforeSubmit', function(e) {
   var form = $(this);
		$.ajax({
            ...
            data   : form.serialize(),
            beforeSend : function()    {
            	$('#submitCustomWordingLoginBottom').attr('disabled','disabled');
            },
            success: function (response){
				if(response == 'OK')
             		CustomWordingLoginBottom();
            	else{
                	$('#submitCustomWordingLoginBottom').removeAttr("disabled");
            	}
            },
            error  : function (){
				...
				<?= IsataskFormFlag::widget(); // change flag ?>
					<div class="x_panel custom-wording-editer">
                            <div class="x_content">
                                <div id="alerts"></div>
					<?php 
		 							if(isset($loginpage_bottom->fieldtext) && $loginpage_bottom->fieldtext != ''){ 
		 								$model->loginpage_bottom = $loginpage_bottom->fieldtext;
		 							}
		 							echo $form->field($model, 'loginpage_bottom',['inputOptions'=>['style'=>"display:block;width:100%"]])->textArea(['rows'=>5,'maxlength'=>$settings_length['fieldtext']])->label(false);
	 							?>
					</div>
					</div>			 	
					<!--<div class="x_panel custom-wording-editer">
                            <div class="x_content">
                                <div id="alerts"></div>
                                <div class="btn-toolbar editor toolbar-justified" data-role="editor-toolbar" data-target="#editor1">
                                    <div class="btn-group">
                                    ...
                                    <div class="btn-group">
										...
										<div class="dropdown-menu input-append"><input type="text" data-edit="createLink" placeholder="URL" id="createLink_txt" class="span2"><label for="createLink_txt">&nbsp;</label><button type="button" class="btn">Add</button></div>
                                    </div>
                                                                    <div class="btn-group">
								...
								<?php 
		 							...
		 							echo $form->field($model, 'loginpage_bottom',['inputOptions'=>['style'=>"display:none;"]])->textArea(['rows'=>5,'maxlength'=>$settings_length['fieldtext']])->label(false);
	 							?>
                                <div id="editor1" class="editor-container">
                                    <?php echo $model->loginpage_bottom;?>
                                </div>
                                <br />
                            </div>
                	</div>-->
					<div class="button-set text-right">
						<?= Html::button('Cancel', ['title'=>"Cancel",'class' => 'btn btn-primary','onclick'=>"javascript:CustomWordingLoginBottomSelect();"]) ?>
             	 		<?= Html::submitButton('Update', ['title'=>"Update",'class' => 'btn btn-primary','id'=>'submitCustomWordingLoginBottom']) ?>
					</div>
				<?php ActiveForm::end(); ?>
<script>
	/* change evnet */
	...
	$(function () {
		  ...
		  $('#settings-loginpage_bottom').jqte({source: false});
...
//			});
		...
		$('#editor').unbind('DOMSubtreeModified').bind('DOMSubtreeModified', function(event) {
			$('#settings-loginpage_bottom').html($(this).html());
		});
		$('#editor1').unbind('DOMSubtreeModified').bind('DOMSubtreeModified', function(event) {
			$('#settings-loginpage_bottom').html($(this).html());
		});	
		
		$('#editor1').wysiwyg('document').keypress(function(e) {			
			// This will limit the charctors to maxlength
			var maxlength = parseInt($('#settings-loginpage_bottom').attr("maxlength"));		 
