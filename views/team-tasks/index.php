...
<div class="button-set text-right" style="min-height:56px;">
	<?php $current_url = Url::current(); ?>
		...
		<?= Html::button('Export',['title'=>"Export",'class' => 'btn btn-primary', 'onclick' => 'export_team_task();'])?>
			<?php } ?>
     </div>
...
function export_team_task() {
    $('#team-task-form').html(null);
		$('#team-task-form').html("<table>"+$('#media_container').find('.table-responsive').find('tr.filters').html()+"</table><input type='hidden' name='_csrf' value='<?=Yii::$app->request->getCsrfToken()?>' /><input type='hidden' name='team_id' id='hidden_team_id' value='<?php echo $team_id; ?>'><input type='hidden' name='team_loc' id='hidden_team_loc' value='<?php echo $team_loc; ?>'>");
		setTimeout(function(){
