...
<div class="right-main-container" id="caseproduction_container">
    ...
    <div class="button-set text-right">
        <input type="text" id="search_doc" title="Search" name="seach_doc" class="form-control" />
        <?= Html::button('Search',['title'=>"Go",'class' => 'btn btn-primary','onclick'=>'search_doc();'])?>
            ...
            if(data=='folder')
            {
                ...
                ref.edit(sel,selected_text,function(node,status){
                        ...
                        $.ajax({
                            ...
                            cache: false,
                            success:function(data){
                                alert('The folder, "'+selected_text+'"  has been renamed to "'+ node.text+'".');
                               // $('#tree').jstree(true).refresh();
                                setTimeout(function(){
        ...
        success:function(data){
            data=data.replace(/\s/g, '');
                if(data=='folder')
                {
                    ...
                    if(confirm('Are you sure you want to Delete "'+selected_text+'" folder?  All subsequent folders and files will also be Deleted.'))
                    {
                        $("#tree").jstree("remove");
                        sel = ref.get_selected();
                        sel = sel[0];
                    ...
                    if(confirm('Are you sure you want to Delete "'+selected_text+'"?'))
                    {
                         $("#tree").jstree("remove");
                         sel = ref.get_selected();
                            sel = sel[0];
...
$('#copy_folder').click(function () 
{
    ...
    $('#cut_selected').val(0);
    if(type == 0)
        alert('The file, "'+selected_text+'" has been copied.');
    else
        alert('The folder, "'+selected_text+'" has been copied.');
});
...
$('#cut_folder').click(function () 
{
    ...
    //$('#copy_selected_childs').val(children1);
    if(type == 0)
        alert('The file, "'+selected_text+'" has been cut.');
    else
        alert('The folder, "'+selected_text+'" has been cut.');
});
$("#permission_folder").click(function () 
{
    ...
    var type=$('#'+selected_node).attr('data-type');
    if(type == 0)
        title="Edit Permissions of '"+selected_text+"' File";
    else
        title="Edit Permissions of '"+selected_text+"' Folder";
