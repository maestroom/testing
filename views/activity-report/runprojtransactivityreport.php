...
<?php
	...
	<?php
			$taskstatus = "Active";  
			if($activity_report[0]['task_status'] == 4)
				$taskstatus = "Completed";
			if($activity_report[0]['task_status'] == 1)
				$taskstatus = "Closed";
			if($activity_report[0]['task_status'] == 1)
				$taskstatus = "Canceled";
	 ?>
...
<script type="text/javascript">
	var status = 'Project Status : '+'<?php echo $taskstatus ?>';
	$('#project_status_id').html(status);
	
	
	
	function my_reportpdf() {
    
       var task_id = '<?php echo $activity_report[0]['id'] ?>';
	
       location.href = baseUrl + "pdf/runmyreport&task_id=" + task_id;
    }
</script>
<noscript></noscript>
