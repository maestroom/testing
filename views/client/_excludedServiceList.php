...
<?php $form = ActiveForm::begin(['id' => 'excludecaseservices-form','enableAjaxValidation' => false,'enableClientValidation' => true]); ?>
	...
	<div class="user-access-mycase" style="top:10px!important">
		...
		<div id="treeclientxteam" class="tree-class"></div>
	...
	<?php /* echo GridView::widget([
		...
		'columns' =>[
			...
			['class' => '\kartik\grid\CheckboxColumn', 'rowHighlight' => false,'headerOptions'=>['title'=>'Select All/None','class'=>'first-th','id'=>'client_management_checkbox','scope'=>'col'] ,'name' => 'excludedservicelist', 'checkboxOptions' => function($model, $key, $index, $column) { 
				return [ 
					'checked' => $model['isserviceexcluded'], 
					'customInput'=>true,
					'class'=>'ckeck_excluded',
					'data-teamservice_id'=>$model['id'],
					'data-team_loc'=>$model['teamservice_loc'], 
					'value' => json_encode(array('client_id'=>$model['client_id'],'team_loc'=>$model['teamservice_loc'],'teamservice_id'=>$model['id'])),
					'aria-label'=>$model['teamservices']]; } 
				],
		 ],
		'floatHeader'=>true,
		'pjax'=>true,
...
<div class="button-set text-right">
	<?= Html::button('Update', ['title' => 'Update','class' =>  'btn btn-primary','onclick'=>'postExcludedClientsServies("excludecaseservices-form",this,"loadExcludedClientServiceList()","excludeserviceform_div");']) ?>
</div>
<?php ActiveForm::end(); ?>
<script>
$('input').customInput();
...
$("#btnDeselectAll").click(function(){
        $("#treeclientxteam").dynatree("getRoot").visit(function(node){
            node.select(false);
        });
        return false;
    });
