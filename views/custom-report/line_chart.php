...
<div class="chart-box" id="chart-box">
	...
	<?php  if(!empty($report_data)){?>
		...
		<?php if(strtolower($modeReportsChartFormat->chart_format)=='line clustered'){
  ...
  <thead>
    ...
    <th colspan="3" id="table_title"><b><b><?=ucwords($post_data['ReportsUserSaved']['y_fn'])?> of <?=$ytable_fields_detail['field_display_name']?> by <?=$series?></b></th>
  </thead>
</table><?php }?>
		<table id="chart_datatable" class="table" style="white-space: nowrap;" >
				<?php  $total=0;
				if(strtolower($modeReportsChartFormat->chart_format)=='line clustered'){?>
					<?=$this->render('line_clustered_stack_table',['post_data'=>$post_data,'report_data'=>$report_data,'table_fields_detail' =>$table_fields_detail,'query'=>$query,'modeReportsChartFormat'=>$modeReportsChartFormat,'ytable_fields_detail'=>$ytable_fields_detail]);?>
				<?php }else{?>
				<thead>
				<tr>
...
var datalabel_align='center';
var datalabel_valign='bottom';
<?php  if(isset($post_data['ReportsUserSaved']['data_label_location']) && $post_data['ReportsUserSaved']['data_label_location']==''){?>
datalabel_enable=false;
<?php }?>
<?php  if(isset($post_data['ReportsUserSaved']['data_label_location']) && $post_data['ReportsUserSaved']['data_label_location']=='Below'){?>
datalabel_enable=true;
datalabel_align='center';
datalabel_valign='top';
<?php }?>
<?php  if(isset($post_data['ReportsUserSaved']['data_label_location']) && $post_data['ReportsUserSaved']['data_label_location']=='Left'){?>
datalabel_enable=true;
datalabel_align='right';
datalabel_valign='top';
<?php }?>
<?php  if(isset($post_data['ReportsUserSaved']['data_label_location']) && $post_data['ReportsUserSaved']['data_label_location']=='Right'){?>
datalabel_enable=true;
			 ...
			 plotOptions: {
				series: {
					dataLabels: {
						...
						verticalAlign: datalabel_valign,
						style:{
							fontFamily: 'Arial',
							fontSize:'12px',
							lineHeight: '34px',
				...
				labels: {
					...
					formatter: function () {
						<?php if($post_data['ReportsUserSaved']['y_fn_display'] == 'Percentage') { ?>
							//return this.value + '%';
							<?php if (strpos(strtolower($modeReportsChartFormat->chart_format),"clustered") !== false) {?>
								//var pcnt = (this.value / this.total) * 100;
								//return Highcharts.numberFormat(pcnt) + ' %';
								return this.value;
							<?php } else { ?>
								var pcnt = (this.value / <?= $total ?>) * 100;
                        		return parseInt(Highcharts.numberFormat(pcnt))+ ' %';
							<?php } ?>
						<?php } else {?>	
							return this.value;
						<?php }?>
