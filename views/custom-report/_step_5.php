...
<fieldset class="one-cols-fieldset-report overflow-auto panel-body">
	<div class="administration-form format_and_properties">
		...
		<?php if(isset($modeReportsChartFormat->chart_format) && strtolower($modeReportsChartFormat->chart_format)=='column basic'){?>
			<?=$this->renderAjax('column_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
		<?php if(isset($modeReportsChartFormat->chart_format) && strtolower($modeReportsChartFormat->chart_format)=='column clustered'){?>
			<?=$this->renderAjax('columnclusteredstack_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
		
		<?php if(isset($modeReportsChartFormat->chart_format) && strtolower($modeReportsChartFormat->chart_format)=='bar basic'){?>
			<?=$this->renderAjax('bar_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
		<?php if(isset($modeReportsChartFormat->chart_format) && strtolower($modeReportsChartFormat->chart_format)=='bar clustered'){?>
			<?=$this->renderAjax('barclusteredstack_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
		
		<?php if(isset($modeReportsChartFormat->chart_format) && strtolower($modeReportsChartFormat->chart_format)=='line basic'){?>
			<?=$this->renderAjax('line_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
		<?php if(isset($modeReportsChartFormat->chart_format) && strtolower($modeReportsChartFormat->chart_format)=='line clustered'){?>
			<?=$this->renderAjax('lineclusteredstack_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
		<?php if(isset($modeReportsChartFormat->chart_format) && (strtolower($modeReportsChartFormat->chart_format)=='circle pie' || strtolower($modeReportsChartFormat->chart_format)=='circle donut')){?>
			<?=$this->renderAjax('pie_sammary',['post_data'=>$post_data,'field_disply_with_table'=>$field_disply_with_table,'reportTypeFields'=>$reportTypeFields]);?>
		<?php }?>
...
<div class="button-set text-right">
	...
	<?= Html::button('Cancel', ['title'=>'Cancel','class' => 'btn btn-primary','onclick'=>'$( "#tabs-step-2" ).show();$( "#tabs-step-1" ).hide();$( "#tabs-step-4" ).hide();$( "#tabs-step-5" ).hide();post_step();']) ?>
...
function next_step_view(){
	...
	if($('#series')){
		if($('#series').val()==""){
			$('#series').parent().parent().parent().addClass('has-error');
			err=true;
		}
	}
	if($('#series1_display_by')){
		if($('#series1_display_by').val()==""){
			$('#series1_display_by').parent().parent().parent().addClass('has-error');
			err=true;
		}
	}
	...
	if($('#series')){
		$('#series').parent().parent().parent().removeClass('has-error');
	}
	if($('#series1_display_by')){
		$('#series1_display_by').parent().parent().parent().removeClass('has-error');
	}
	...
	$.ajax({
			type: 'post',
			url:baseUrl+'custom-report/view-chart',
			data: form,
