...
<?php
$is_format_number=false;
$is_format_number_with_comma=false;
if(isset($post_data['grouping_value']) && $post_data['grouping_value']!=""){
		foreach($post_data['grouping_value'] as $gv){
			$data_gv=json_decode($gv,true);
			if($data_gv['id']==$post_data['ReportsUserSaved']['x_data'] && $data_gv['group-display-by']==2){
				$is_format_number=true;
				if(isset($data_gv['group-display-number-sp'])){
					$is_format_number_with_comma=true;
				}
				break;
			}
		}
...
if(!isset($table_fields_detail['field_display_name'])){
	if($modelreportsReportType->sp_name='MediaOut')
		$table_fields_detail['field_display_name'] = isset($post_data['fielddisp'][$post_data['ReportsUserSaved']['x_data']])?$post_data['fielddisp'][$post_data['ReportsUserSaved']['x_data']]:'TotalMediaOut('.$post_data['ReportsUserSaved']['x_data'].')';
	else
		$table_fields_detail['field_display_name'] = isset($post_data['fielddisp'][$post_data['ReportsUserSaved']['x_data']])?$post_data['fielddisp'][$post_data['ReportsUserSaved']['x_data']]:$post_data['ReportsUserSaved']['x_data'];
}
...
<div class="chart-box" id="chart-box">
	<div id="dynamic_chart" class="clsperiodchart chart-container" style="<?php if($post_data['ReportsUserSaved']['datatable_location']=='R' && $post_data['ReportsUserSaved']['legend_location']=='R'){?>width:95%;<?php } else if($post_data['ReportsUserSaved']['datatable_location']=='R' && $post_data['ReportsUserSaved']['legend_location']=='B') { ?> float:left;width:75%;<?php }else{?> width:95%; <?php } ?>"></div>
	...
	<?php  if(!empty($report_data)){ ?>
		<h6 class="font14" align="<?php if($post_data['ReportsUserSaved']['title_location']=='TL'){?>left<?php }else { ?>center<?php }?>" style="<?php if($post_data['ReportsUserSaved']['title_location']=='TL'){?>margin-left:10px;<?php }?>"><strong>Data Table</strong></h6>
			<?php if(strtolower($modeReportsChartFormat->chart_format)=='bar clustered'){
			$series = $post_data['fielddisp'][$post_data['ReportsUserSaved']['series']];
		?>
		<table class="table_step6" style="white-space: nowrap;">
  <thead>
    <th width="30%">&nbsp;<span class="not-set">Tabular Report</span></th>
    <th colspan="3" id="table_title"><b><b><?= ucwords($post_data['ReportsUserSaved']['x_fn']) ?> of <?= $table_fields_detail['field_display_name'] ?> by <?= $series ?></b></th>
  </thead>
</table><?php }
if($post_data['ReportsUserSaved']['legend_location'] == 'R' && $post_data['ReportsUserSaved']['datatable_location']=='R')
	$tbl_style = "";
else
	$tbl_style = "white-space: nowrap;";
?>
	<table id="chart_datatable" class="table" style="<?php echo $tbl_style;?>" >
			<?php 
				if(strtolower($modeReportsChartFormat->chart_format)=='bar clustered'){?>
					<?= $this->render('bar_clustered_stack_table',['post_data'=>$post_data,'report_data'=>$report_data,'table_fields_detail' =>$table_fields_detail,'query'=>$query,'modeReportsChartFormat'=>$modeReportsChartFormat,'ytable_fields_detail'=>$ytable_fields_detail]); ?>
			<?php } else { ?>
				<thead>
					<tr>
						<th width="30%"><b><?=ucwords($ytable_fields_detail['field_display_name'])?></b></th>
						<th><b><?=ucwords($post_data['ReportsUserSaved']['x_fn'])?> of <?=$table_fields_detail['field_display_name']?></b></th>
					</tr>
				</thead>
				<?php foreach($report_data as $rdata) {
						if(isset($post_data['ReportsUserSaved']['y_data_display']) && ($post_data['ReportsUserSaved']['y_data_display']=='Weeks' || $post_data['ReportsUserSaved']['y_data_display']=='Days' || $post_data['ReportsUserSaved']['y_data_display']=='Months' || $post_data['ReportsUserSaved']['y_data_display']=='Years')){}else{if(!isset($rdata[1])){continue;}}
					?>
					<tr>
						<td>
							<?php if(isset($post_data['ReportsUserSaved']['y_data_display']) && ($post_data['ReportsUserSaved']['y_data_display']=='Weeks')){?>
							<?=date("m/d/Y",strtotime($rdata['start_date']))."-".date("m/d/Y",strtotime($rdata['end_date']))?>
							<?php }elseif(isset($post_data['ReportsUserSaved']['y_data_display']) && ($post_data['ReportsUserSaved']['y_data_display']=='Days')){?>
								<?php echo '"'.date("m/d/Y",strtotime($rdata['start_date'])).'"';?>
							<?php }elseif(isset($post_data['ReportsUserSaved']['y_data_display']) && ($post_data['ReportsUserSaved']['y_data_display']=='Months')){?>
								<?php echo date("m/Y",strtotime($rdata['start_date']));?>
							<?php }elseif(isset($post_data['ReportsUserSaved']['y_data_display']) && ($post_data['ReportsUserSaved']['y_data_display']=='Years')){ ?>
						...
						<td>
							<?php if(isset($post_data['ReportsUserSaved']['y_data_display']) && ($post_data['ReportsUserSaved']['y_data_display']=='Weeks' || $post_data['ReportsUserSaved']['y_data_display']=='Days' || $post_data['ReportsUserSaved']['y_data_display']=='Months' || $post_data['ReportsUserSaved']['y_data_display']=='Years')){?>
								<?=$rdata['X']?>
							<?php } else { ?>
								<?= $rdata[0]?>
							<?php }?>
...
<?php }?>
<?php if(strtolower($modeReportsChartFormat->chart_format)=='bar clustered'){?>
var chart_height = $('#datatable_section tr').length * 60 + 120;
<?php } else {?>
var chart_height = $('#datatable_section tr').length * 20 + 120;
<?php } /*if($is_format_number==true){?>
...
<?php } if($is_format_number_with_comma==true){?>
Highcharts.setOptions({
        global: {
            useUTC: false,
...
<?php }*/?>
	var options = {
			chart: {
				...
				events: {
					redraw: function(event) {
						...
						if(title_align == 'left') {
							this.options.title.x = 1;
						}

						if(title_align == 'center') {
							<?php if($post_data['ReportsUserSaved']['datatable_location']=='B'){ ?>
								this.options.title.x = parseInt((parseInt(this.plotLeft)+parseInt(this.plotWidth))/2);
							// (parseInt($('#datatable_section').width())/2)-22;
							<?php } else { ?>
								this.options.title.x = parseInt((parseInt(this.plotLeft)+parseInt(this.plotWidth))/2);
							<?php } ?>
						...
						//$('.highcharts-legend').css("left", parseInt(this.plotLeft)+parseInt(this.plotWidth)+20+"px");
						...
						//$('#datatable_section').css("width",parseInt(this.options.legend.width));
						//this.options.legend.width = $('#datatable_section').width()-10;
						//alert($('#datatable_section').width());
							$('.highcharts-legend-box').attr("width", $('#datatable_section').width()+"px");
							$('.highcharts-legend').attr("width", $('#datatable_section').width()+"px");
						...
						if(legend_location_verticalAlign == 'bottom'){
							...
							this.options.chart.height = chart_height;
						}
			...
			xAxis: {
				...
				title: {
					enabled: true,
					text: '<?= ucwords($ytable_fields_detail['field_display_name']) ?>',
					style: {
						fontFamily: 'Arial',
						fontSize:'12px',
						fontWeight: 'bold',
				...
				formatter: function () {
					<?php if (strpos(strtolower($modeReportsChartFormat->chart_format),"clustered") !== false) { ?>
						<?php if($post_data['ReportsUserSaved']['x_fn_display'] == 'Currency') { ?>
							return '<span style="font-family:Arial;font-size:11px;line-height:34px;margin: 0;"><b><?=ucwords($post_data['ReportsUserSaved']['x_fn'])?> of '+this.key+' - '+this.series.legendItem.textStr+': $' + number_format(this.y,2) +'</b></span>';
						<?php } else { ?>
							return '<span style="font-family:Arial;font-size:11px;line-height:34px;margin: 0;"><b><?= ucwords($post_data['ReportsUserSaved']['x_fn']) ?> of '+this.key+' - '+this.series.legendItem.textStr+': ' + this.y +'</b></span>';
						<?php } ?>
					<?php } else { ?>
						<?php if($post_data['ReportsUserSaved']['x_fn_display'] == 'Currency') { ?>
							return '<span style="font-family:Arial;font-size:11px;line-height:34px;margin: 0;"><b><?=ucwords($post_data['ReportsUserSaved']['x_fn'])?> of '+this.key+': $' + number_format(this.y,2) +'</b></span>';
						<?php } else { ?>
							return '<span style="font-family:Arial;font-size:11px;line-height:34px;margin: 0;"><b><?=ucwords($post_data['ReportsUserSaved']['x_fn'])?> of '+this.key+': ' + this.y +'</b></span>';
						<?php } ?>
					<?php }?>
				}
			},
			legend: {
				...
				labelFormatter: function() {
					...
					return this.name;
					<?php }else{ ?>
					return 	'<?=$table_fields_detail['field_display_name']?>';                    
					<?php }?>
				},
				itemStyle: {
					fontFamily: 'Arial',
			...
			yAxis: {
				...
				title: {
					enabled: true,
					text: '<?=ucwords($post_data['ReportsUserSaved']['x_fn'])?> Of <?=$table_fields_detail['field_display_name']?>',
					style: {
						fontFamily: 'Arial',
						fontWeight: 'bold',
						fontSize:'12px',
