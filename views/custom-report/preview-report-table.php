...
<table class="table table-striped" id="custom_report_preview_table">
	<thead>
		<tr>
			<?php $column_names = array();foreach($column_display_data as $key => $column){ 
			if($column=='task_unit_id'){$column_names[$key] =$column_data[$key];continue;}
			$fieldexplode = explode(".",$column_data[$key]); if($fieldexplode[0]=='Calc'){ $column_names[$key] = str_replace(" ","_",strtolower($fieldexplode[1])); } else {if(isset($fieldexplode[1])){$column_names[$key] = $fieldexplode[1];}else {$column_names[$key] =$column_data[$key];} }?>
	...
	<tbody>
		<?php 
		//echo "<pre>",print_r($column_names),print_r($column_display_data),"</pre>";
		$taskunitid_key=9999999999;
		if(array_search("task_unit_id",array_values($column_display_data))){
			$taskunitid_key=array_search("task_unit_id",array_values($column_display_data));
		}
				...
				<?php 
					if(isset($reportTypeFields[$col_type]) && $reportTypeFields[$col_type] == 'DATETIME'){
						...
						if($format_data[$col_type]) {
							...
							echo html_entity_decode(htmlspecialchars($data[$i]));
						}
					}
					}
				?>
