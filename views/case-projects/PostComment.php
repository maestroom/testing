...
<div class="right-main-container">
			...
			<?php $form = ActiveForm::begin(['id' => $model->formName(),'enableAjaxValidation' => false,'enableClientValidation' => true]); ?>
			 ...
			 <div class="comments-area">
             	...
             	<!--<div class="x_panel custom-wording-editer">
                            <div class="x_content">
                                ...
                                <div id="editor" class=" editor-container" aria-label='Leave a comment' contentEditable=true data-text="Leave a comment..."></div>
                                </div>
                        </div>-->
			 ...
			 <div class="button-set text-right">
			  ...
			  <button class="btn btn-primary" title="Add Recipients" type="button" onclick="Recipients()" id="recipients" >Add Recipients</button>
		...
		<div id="comments" class="comments left case_88">
			 <?=$this->render('_listComment', ['comment_data'=>$comment_data,'task_id'=>$task_id,'case_id'=>$case_id,'model'=>$model]);?>
		</div>
	</fieldset>
</div>
<script>
...
function Recipients()
{
    ...
    var url = baseUrl + "case-projects/newrecipients";
    ...
    $.ajax({
	    type: "post",
	    data:{task_id:<?php echo $task_id?>,case_id:<?php echo $case_id?>,team_ids:team_ids,case_ids:case_ids,teams_ids:teams_ids,cases_ids:cases_ids,fixed_emailsend_user_ids:fixed_emailsend_user_ids},
		url: url,
...
function Recipients_OLD()
{
    ...
    var url = baseUrl + "case-projects/recipients";
    var team_ids= $("#team_ids").val();
    var case_ids=	$("#case_ids").val();
    var teams_ids= $("#teams_ids").val();
    var cases_ids=	$("#cases_ids").val();
    $.ajax({
	    type: "post",
	    data:{task_id:<?php echo $task_id?>,case_id:<?php echo $case_id?>,team_ids:team_ids,case_ids:case_ids,teams_ids:teams_ids,cases_ids:cases_ids},
		url: url,
		success:function(data){
			$( "#comment-recipients" ).html(data);
			$( "#comment-recipients" ).dialog('open');
...
function comment_recipients_inner(case_roles, team_roles)
{
	...
	$( "#comment-recipients-inner" ).dialog({
	    ...
	    open:function() {
	    	$('.ui-dialog :button').blur();
			$('.ui-dialog-titlebar .ui-dialog-titlebar-close').each(function(){
				$(this).html('<span class="ui-button-icon-primary ui-icon"></span>');
                                $(this).attr("aria-label", "Close");
			})
		},
	});
    
    var url = baseUrl + "case-projects/recipients-users";
    var team_ids= $("#team_ids").val();
    var case_ids = $("#case_ids").val();
    $.ajax({
	    type: "post",
