			...
			<?php $form = ActiveForm::begin(['id' =>"edit-comments-form-".$model->Id,'enableAjaxValidation' => false,'enableClientValidation' => true]); ?>
			 ...
			 <div class="comments-area">
				...
				<div class="x_panel custom-wording-editer">
                            <div class="x_content">
				 <?= $form->field($model, 'comment',['inputOptions'=>['style'=>"display:block;width:100%"],'template' => "<div class='col-md-0'>{label}</div><div class='col-md-12' style='padding:0px!important;'>{input}\n{hint}\n{error}</div>",'labelOptions'=>['class'=>'form_label']])->textArea(['id'=>'edit_comment_'.$model->Id,'rows'=>2,'placeholder'=>"Leave a comment...",'value'=>htmlentities($model->comment,ENT_COMPAT,'UTF-8')])->label(false);?>
				 </div>
				 </div>
				 	...
				 	<div class="col-sm-12">
				 		<div class="col-sm-7">
				  	 		<div class=" MultiFile-title">
								<span> 
								<?= $form->field($model, 'attachment[]',['template' => "<div class='row input-field'><div class='col-md-4'>{label}</div><div class='col-md-8'>{input}<div id='comments-attachment_list' class='MultiFile-list text-left'>(File Size can't exceed 100MB)</div>\n{hint}\n{error}</div></div>",'labelOptions'=>['class'=>'form_label','label'=>'Attachment']])->fileInput(['multiple' => true,"id"=>uniqid(), 'class'=>'multi-pt','title'=>'Choose File']) ?>
								<?php
								...
								if (!empty($model->attachments)) {
					               ...
					               <div class="MultiFile-label selected-file-list text-left attach_<?php echo $model->Id?>" id="attach_<?php echo $filename->id; ?>">
					                   <a href="#T7" class="MultiFile-remove" onclick="edit_remove_image('<?php echo $filename->id; ?>', this,'<?php echo $model->Id?>');" title="Delete attachment"><em title="Remove" class="fa fa-close text-danger"></em></a>
...
$('document').ready(function(){
	console.log('edit-comments-form-<?= $model->Id ?>');
	$('#active_form_name').val('edit-comments-form-<?= $model->Id ?>'); // active form name
});
