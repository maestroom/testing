...
$('.my_secondary_header a').on('click',function(){
	if($(this).parent().hasClass('myheader-selected-tab')){
		$(this).parent().removeClass('myheader-selected-tab');
	} else {
...
//                $('#table_primary_relation_'+unique_single_id).next().removeClass('checked');
	...
	$('#availabl-second-tables').on('scroll', function() {
        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
            var total_table_count = $('.tbl_total_loaded').val();
            var table_loaded = $('.tbl_already_loaded').val();
            if(total_table_count < 101){
	            jQuery.ajax({
	                url: baseUrl +'/report-management/get-more-second-table-lists',
	                type: 'POST',
	                data: {total_table_count:total_table_count,table_loaded:table_loaded},													
	                beforeSend:function (data) {showLoader();},
	                success: function (data) {															
	                    hideLoader();						
	                    $('#availabl-second-tables .mycontainer').append(data);										
	                    $('.tbl_total_loaded').val(parseInt(total_table_count)+25);
		            }
		        });	
        	}
        }
...
foreach($tables_list as $tbl_fields) {
    ...
    $unique_id=uniqid();
    ?> 
    <div class="myheader my_secondary_header">
...
<?php   } } }?>
    <input type="hidden" class="tbl_secondary_name" value="">    
    <input type="hidden" class="tbl_total_loaded" value="25">
    <input type="hidden" class="tbl_already_loaded" value="<?=base64_encode(json_encode($post_data))?>">
</div>