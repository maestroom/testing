...
<div class="right-main-container" id="media_container">
	<div class="">
	    <fieldset class="two-cols-fieldset workflow-management">
	       ...
	       <?php $form = ActiveForm::begin(['id' => 'data_process_report','action' => '@web/index.php?r=processing-report/data-processing']); ?>
...
<script type="text/javascript">    
	...
	//	$("#dataexport").val("export");
			$("#data_process_report").attr("action", baseUrl + 'processing-report/exportdataclientcase');
			console.log($("#data_process_report").attr("action"));
			$("#data_process_report").submit();
	//	$("#dataexport").val("");
	}
    ...
    function back(){
		$("#data_process_report").attr("action", httpPath + 'processing-report/data-processing');
		$("#data_process_report").submit();
	}
...
<?php if (isset($clientchartSeries)) { // && $clientchartSeries != "" && $clientchartSeries != "[]" ?>
        createclientcasechart(<?php echo $clientchartCategories ?>,<?php echo $clientchartSeries ?>,'<?php echo 'Data Processed By Selected '.$select_client_case.' (GB) - '.$element_name; ?>');
<?php } else { ?>
        noreport();
<?php } ?>	
</script>
