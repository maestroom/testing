 ...
 * @property integer $cnttasksbypriority
 * @property string $priority
 */
class TasksUnits extends \yii\db\ActiveRecord
{
	public $totalunits, $task_priority,$team_priority,$client_id,$client_case_id,$project_name,$task_date_time,$task_closed,$task_cancel,$ispastdue,$cnttasksbypriority,$priority;
	public $todo_project_name, $tasks_unit_id, $todo_client_id, $todo_client_case_id, $todo, $todo_cat, $todo_assigned;
	...
	public function getTaskPendingServiceTaskCount(){
		$userId = Yii::$app->user->identity->id;
		$service_ids = array();
		$timezoneOffset = (new Options)->getOffsetOfCurrenttimeZone();
		if (Yii::$app->db->driverName == 'mysql') {
		...
		$sql = "Select COUNT(*) as cntpendingtasks From (
		Select A.* From (
...
//			    WHERE t.unit_status=0 AND t.sort_order=1 AND task.task_closed=0 AND task.task_cancel=0 AND Taskindividual.isactive=1 AND clientCase.is_close=0 AND task.task_closed=0 AND t.team_id = ".$team_id." AND t.team_loc = ".$team_loc.") AS B GROUP BY B.task_unit_id";
	...
	/* Function for Not Started Task Count for My Case and My Team Assignment
	 * */

	public function getTaskNotstartedTaskCount(){
...
$taskduedatejoin = " LEFT JOIN (SELECT tbl_task_instruct.id, $data_query as task_date_time FROM tbl_task_instruct WHERE isactive=1 AND task_id IS NOT NULL AND task_id<>'') as C ON Taskindividual.id = C.id ";
    $final_data = "SELECT COUNT(*) as cntnotstarted  FROM tbl_tasks_units as t
    LEFT JOIN tbl_task_instruct Taskindividual ON (Taskindividual.id=t.task_instruct_id)
	$taskduedatejoin
    INNER JOIN tbl_tasks task ON (Taskindividual.task_id=task.id)
    LEFT JOIN tbl_client_case clientCase ON (task.client_case_id=clientCase.id)
...
//        print_r($mainarra);die;
    ...
    {
        return $this->hasOne(TasksUnitsTodos::className(), ['tasks_unit_id' => 'id']);
    }
