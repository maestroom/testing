...
class TasksUnitsSearch extends TasksUnits
{
    ...
    public function rules()
    {
        return [
            ...
            [['id','duration', 'unit_complete_date', 'created', 'modified','task_id','unit_assigned_to','unit_status','workflow_task','task_priority','team_priority','client_id','client_case_id','project_name','task_duedate'], 'safe'],
...
/**
    ...
    public function searchMyActiveTasks($params)
    {
    	...
    	if (Yii::$app->db->driverName == 'mysql'){
			...
			$sqlpastdue.=" AND task_date_time < CASE WHEN tbl_tasks.task_complete_date!='0000-00-00 00:00:00' AND tbl_tasks.task_complete_date!='' AND tbl_tasks.task_status = 4 THEN '$task_duedatetime' ELSE '$todaysdate' END";
        }else{
			$task_completedate = " CAST(switchoffset(todatetimeoffset(tbl_tasks.task_complete_date, '+00:00'), '$timezoneOffset') as DATETIME) ";
        	$sqlpastdue.=" AND task_date_time < CASE WHEN CAST(tbl_tasks.task_complete_date as varchar)!='0000-00-00 00:00:00' AND CAST(tbl_tasks.task_complete_date as varchar) IS NOT NULL AND tbl_tasks.task_status = 4 THEN '$task_duedatetime' ELSE '$todaysdate' END";
		}
		...
		if ($params['TasksUnitsSearch']['priority'] != null && is_array($params['TasksUnitsSearch']['priority'])) {
			if(!empty($params['TasksUnitsSearch']['priority'])){
				foreach($params['TasksUnitsSearch']['priority'] as $k=>$v){
					if($v=='All'){ //  || strpos($v,",") !== false
						unset($params['TasksUnitsSearch']['priority']);
					}
				}
			}
		}
    	...
    	if(isset($params['TasksUnitsSearch']['unit_status']) && $params['TasksUnitsSearch']['unit_status']!="" && $params['TasksUnitsSearch']['unit_status']!="All")
			$query->andFilterWhere(['tbl_tasks_units.unit_status'=>$params['TasksUnitsSearch']['unit_status']]);
		//else 	
    	...
    	if(isset($params['TasksUnitsSearch']['priority']) && $params['TasksUnitsSearch']['priority']!="" && $params['TasksUnitsSearch']['priority']!="All"){
			$query->andFilterWhere(['or like', 'priority', $params['TasksUnitsSearch']['priority'],false]);
		}
	...
	public function searchMyActiveTasksFilter($params)
    {
		...
		$query->joinWith(['taskInstruct'=>function (\yii\db\ActiveQuery $query){
			$query->joinWith(['taskPriority'],false);
		}],false);
		$query->joinWith(['servicetask']);
		...
		if($params['field']=='task_priority'){
			$dataProvider = ArrayHelper::map($query->all(),
			function($model){
				return $model->taskInstruct->task_priority;
			},
			function($model){
...
and t.assigned =".$params['unit_assigned_to']."";
		...
		if(in_array(7,$status)) {
    		...
    		$sqldatacount = $sqldata = "Select B.* From (
				Select A.* From (
...
//		echo $where;die;
		...
		//		$where.= ' AND tunits.unit_status = TasksUnitsSearch.$status;
			...
			$where.= " AND tunits.unit_status != 4 AD task.task_closed = 0";
		}
